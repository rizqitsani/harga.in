...
class UpdateProductViewModel @Inject constructor(
    ...
    private val state =
        MutableStateFlow<UpdateProductFragmentState>(
            UpdateProductFragmentState.Init
        )
    val mState: StateFlow<UpdateProductFragmentState> get() = state
    ...
    private fun setLoading(isLoading: Boolean) {
        state.value = UpdateProductFragmentState.IsLoading(isLoading)
    }

    private fun showToast(message: String) {
        state.value = UpdateProductFragmentState.ShowToast(message)
    }

    private fun setIsUpdated(isUpdated: Boolean) {
        state.value = UpdateProductFragmentState.IsUpdateSuccess(isUpdated)
    }

    fun updateProduct(productId: String, productUpdateRequest: ProductUpdateRequest) {
        viewModelScope.launch {
            productUseCase.update(productId, productUpdateRequest)
                .onStart {
                    setLoading(true)
                }
                .catch { exception ->
...
sealed class UpdateProductFragmentState {
    object Init : UpdateProductFragmentState()
    data class IsLoading(val isLoading: Boolean) : UpdateProductFragmentState()
    data class ShowToast(val message: String) : UpdateProductFragmentState()
    data class IsUpdateSuccess(val isDeleted: Boolean) : UpdateProductFragmentState()
}